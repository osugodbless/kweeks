...
func (s *Server) Routes(mux *http.ServeMux) {
	...
	mux.HandleFunc("POST /api/wallet/create-user", s.requireAuth(s.handleCreateUser))
	mux.HandleFunc("POST /api/wallet/kyc/lookup", s.requireAuth(s.handleKYCLookup))
	...
	mux.HandleFunc("POST /api/wallet/activate-rail", s.requireAuth(s.handleActivateRail))
	...
	mux.HandleFunc("GET /api/quizzes/{quizID}/results", s.requireAuth(s.handleQuizResults))
...
func writeErr(w http.ResponseWriter, err error) {
	...
	case errors.Is(err, domain.ErrAnswerLate), errors.Is(err, domain.ErrAlreadyAnswered),
		...
		errors.Is(err, domain.ErrNoMoreQuestions), errors.Is(err, domain.ErrQuizAlreadyStarted),
		errors.Is(err, domain.ErrNoQuestions),
