...
func (s *Store) CreateInstructor(ctx context.Context, i *domain.Instructor) error {
	_, err := s.pool.Exec(ctx, `
		insert into instructors (id, name, email, phone, password_hash, avatar, created_at, first_name, last_name)
		values ($1,$2,$3,$4,$5,$6,$7,$8,$9)`,
		i.ID, i.Name, i.Email, i.Phone, i.PasswordHash, i.Avatar, i.CreatedAt, i.FirstName, i.LastName)
	if isUniqueViolation(err) {
...
func scanInstructor(row pgx.Row) (*domain.Instructor, error) {
	...
	if err := row.Scan(&i.ID, &i.Name, &i.Email, &i.Phone, &i.PasswordHash, &i.Avatar, &i.CreatedAt, &i.FirstName, &i.LastName); err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
...
func (s *Store) GetInstructorByEmail(ctx context.Context, email string) (*domain.Instructor, error) {
	return scanInstructor(s.pool.QueryRow(ctx, `
		select id, name, email, phone, password_hash, avatar, created_at, first_name, last_name
